...
Using the `MechaCar_mpg.csv` dataset, we created a linear model for the dataset's columns[^1], and stored the result as `mpg_lm`.
...
<br>
We first note that the specification is malformed[^2], and presume that what is means to say is:

> The variance of the suspension coils' PSI values must not exceed 100 \[units left unstated\].

[^2]: We know this because variance is a *squared* measure, and therefore its units will also be the square of the units of the data it aggregates. In the case of data measured in PSI—pounds per square inch—the units of the variance statistic would be in PSI² (or square pounds per quartic inch), not PSI.
